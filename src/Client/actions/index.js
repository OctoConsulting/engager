...
export function addingEvent({token, enteredDate, type, eventName, description, expiration}){
  const user_id = jwt.decode(token, config.secret);
  const timestamp_n = new Date();
  const timestamp_epoch = Date.parse(timestamp_n) / 1000;

  return function (dispatch){
    axios.put(`${SERVER_URL}/addingEvent/${user_id.sub}`, {timestamp_epoch, type, enteredDate, eventName, description, expiration})
          .then( response => {
            console.log(response.data);
            dispatch({type:EVENTS_INFO, payload:response.data});
          })
